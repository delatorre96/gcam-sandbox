...
MODEL_BASE_YEAR <- 2005
HISTORICAL_YEARS <- 1971:MODEL_BASE_YEAR
...
MODEL_BASE_YEARS <- {yrs <- c(
    1975,
    1990,
    seq(2005, MODEL_BASE_YEAR, by = 5)
  )
  yrs[yrs == 2020] <- 2021
  sort(unique(yrs))
}
...
MODEL_FUTURE_YEARS <- seq(
  MODEL_FINAL_BASE_YEAR + 5,
  2100,
  by = 5
)

MODEL_FUTURE_YEARS[MODEL_FUTURE_YEARS == 2020] <- 2021
MODEL_FUTURE_YEARS <- sort(unique(MODEL_FUTURE_YEARS))
...
aglu.MODEL_PRICE_YEARS      <- seq(MODEL_FINAL_BASE_YEAR - 1,MODEL_FINAL_BASE_YEAR + 1) # consistent with aglu.MODEL_SUA_MEAN_PERIODS
aglu.MODEL_MACRONUTRIENT_YEARS <- aglu.MODEL_PRICE_YEARS   # consistent with aglu.MODEL_SUA_MEAN_PERIODS; FAO only has data for after 2010
aglu.MODEL_COST_YEARS       <-  seq(MODEL_FINAL_BASE_YEAR - 7,MODEL_FINAL_BASE_YEAR - 1)
aglu.DEFLATOR_BASE_YEAR     <- MODEL_FINAL_BASE_YEAR      # year used as the basis for computing regional price deflators
aglu.FALLOW_YEARS           <- aglu.MODEL_PRICE_YEARS # Years used for calculating the % of fallow land
...
if (MODEL_BASE_YEAR < 2010){
  MODEL_CARBON_YEAR <- MODEL_BASE_YEAR
}else{
...
Socioeconomic.PWT.LastYear <- MODEL_FINAL_BASE_YEAR # the latest year in PWT
socioeconomics.Global_Macro_Database_LastYear <- MODEL_FINAL_BASE_YEAR # the latest year in GMD
socioeconomics.GCAM_GDP_Dollar_Year <- 1990 # GDP dollar year in GCAM
socioeconomics.SSP_DB_BASEYEAR <- MODEL_FINAL_BASE_YEAR # base year of SSP data base v3.2 Beta
...
socioeconomics.GDP_ADJ_NO_NEG_GROWTH_YEAR <- MODEL_FINAL_BASE_YEAR # used for adjusting GDP projection to avoid negative GDP growth after this year (for Taiwan and South_America_North)
...
emissions.UNCONVENTIONAL.OIL.FUG.N2O.EMFACT <- 0.000000939
  ...
  emissions.EPA_MACC_YEAR           <- seq(MODEL_FINAL_BASE_YEAR,2050, by = 5)       # based on 2019 EPA nonCO2 report
 ...
 emissions.EPA_BAU_HIST_YEAR <- seq(1990, MODEL_FINAL_BASE_YEAR, by = 5) # based on 2019 EPA nonCO2 report
 emissions.EPA_BAU_HIST_YEAR[emissions.EPA_BAU_HIST_YEAR == 2020] <- 2021
...
if (MODEL_FINAL_BASE_YEAR %in% gcamusa.LOAD_SEG_CAL_YEARS){
  gcamusa.LOAD_SEG_CAL_YEARS <- gcamusa.LOAD_SEG_CAL_YEARS[gcamusa.LOAD_SEG_CAL_YEARS <= MODEL_FINAL_BASE_YEAR]
}

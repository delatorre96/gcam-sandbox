...
module_aglu_L100.FAO_SUA_connection <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    if (MODEL_FINAL_BASE_YEAR < min(FAO_Food_Macronutrient_All$year)){
      all_years <- MODEL_FINAL_BASE_YEAR:max(FAO_Food_Macronutrient_All$year)
      df_minYear <- FAO_Food_Macronutrient_All %>%
        filter(year == min(FAO_Food_Macronutrient_All$year))
      df_backfill <- df_minYear %>%  select(-year) %>%
        tidyr::expand_grid(year = MODEL_FINAL_BASE_YEAR: min(FAO_Food_Macronutrient_All$year) - 1) %>%
        select(names(FAO_Food_Macronutrient_All))
      FAO_Food_Macronutrient_All <- bind_rows(FAO_Food_Macronutrient_All, df_backfill) %>%
        arrange(area_code, item_code, macronutrient, year)

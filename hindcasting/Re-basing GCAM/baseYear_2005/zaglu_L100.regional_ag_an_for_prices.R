...
module_aglu_L100.regional_ag_an_for_prices <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    fill_na_nearest_base <- function(x) {
      n <- length(x)
      for(i in seq_len(n)) {
        if (is.na(x[i])) {
          # Look back
          left <- if(i > 1) max(which(!is.na(x[1:(i-1)]))) else NA
          # Look ahead
          right <- if(i < n) min(which(!is.na(x[(i+1):n])) + i) else NA

          # Decide which side is closer
          dist_left <- ifelse(is.na(left), Inf, i - left)
          dist_right <- ifelse(is.na(right), Inf, right - i)

          if(dist_left <= dist_right && dist_left != Inf) {
            x[i] <- x[left]
          } else if(dist_right != Inf) {
            x[i] <- x[right]
          }
        }
      }
    ...
    years_cols <- colnames(FAO_GDP_Deflators)[7:ncol(FAO_GDP_Deflators)]
    FAO_GDP_Deflators <- FAO_GDP_Deflators
    FAO_GDP_Deflators[years_cols] <- t(apply(FAO_GDP_Deflators[years_cols], 1, fill_na_nearest_base))
    ...
    if (MODEL_BASE_YEAR < 2010){
      df_2005 <- GCAMFAOSTAT_ProdPrice
      df_2005$`2005` <- df_2005$`2010`
      df_2005 <- df_2005[, c(
        "item_code", "item", "area_code", "area", "element", "unit", "2005"
      )]
      GCAMFAOSTAT_ProdPrice$`2005` <- df_2005$`2005`
      year_cols <- sort(as.numeric(grep("^[0-9]{4}$", names(GCAMFAOSTAT_ProdPrice), value = TRUE)))
      fixed_cols <- c("item_code", "item", "area_code", "area", "element", "unit")

      GCAMFAOSTAT_ProdPrice <- GCAMFAOSTAT_ProdPrice[, c(fixed_cols, as.character(year_cols))]

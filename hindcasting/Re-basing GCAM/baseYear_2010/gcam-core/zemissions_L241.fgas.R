...
module_emissions_L241.fgas <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    fill_closest <- function(years, values) {
      # years y values deben ser vectores del mismo tamaño
      out <- values
      na_idx <- which(is.na(values))
      not_na_idx <- which(!is.na(values))

      if(length(not_na_idx) == 0) {
        # todos NA → dejamos como NA
        return(out)
      }

      # Para cada NA, buscamos el índice de valor más cercano
      for(i in na_idx) {
        diffs <- abs(years[i] - years[not_na_idx])
        closest <- not_na_idx[which.min(diffs)]
    ...
    L241.hfc_ef_update_all %>%
      ...
      mutate(emiss.coeff = fill_closest(year, emiss.coeff)) %>%
      ungroup() %>%

...
module_energy_L113.atb_cost <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Change Base Year to extrapolate
   ...
   # Establish some year constants for data processing
    ...
    if(energy.ATB_BASE_YEAR < energy.ATB_LATEST_YEAR) {
      ...
      L113.globaltech_cost_atb %>%
        ...
        gather_years(activate = FALSE) -> L113.extended_data
    ...
    A23.globaltech_capital %>%
      ...
      bind_rows(A23.globaltech_capital_adv  %>%
                ...
                A23.globaltech_OMvar %>%
                  rename(input = input.OM.var) %>%
                  mutate(case = energy.COSTS_MID_CASE)) %>%
      gather_years(activate = FALSE) -> A23.globaltech_cost
    ...
    L113.globaltech_cost %>%
      ...
      select(-c(case, '2020','2021')) -> L113.globaltech_capital_ATB
    ...
    L113.globaltech_cost  %>%
      ...
      rename('input-capital' = input) %>%
      select(-c(case, '2020','2021')) -> L113.globaltech_capital_ATB_adv
    ...
    L113.globaltech_cost %>%
      ...
      rename('input-capital' = input) %>%
      select(-c(case, '2020','2021')) -> L113.globaltech_capital_ATB_low
    ...
    L113.globaltech_cost %>%
      ...
      select(-c(case, '2020','2021'))  -> L113.globaltech_OMfixed_ATB
    ...
    L113.globaltech_cost %>%
      ...
      select(-c(case, '2020','2021'))  -> L113.globaltech_OMvar_ATB

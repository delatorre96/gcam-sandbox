...
#' module_gcamusa_L2247.elecS_tech_costs
#'
...
#' @author RC Sep 2018
module_gcamusa_L2247.elecS_tech_costs <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A23.itc_USA %>%
      ...
      {
        first_valid <- .$itc[which(!is.na(.$itc))[1]]
        .$itc[is.na(.$itc)] <- first_valid
        .
      } ->
      A23.itc_USA
    ...
    A23.ptc_USA %>%
      ...
      {
        first_valid <- .$input.cost[which(!is.na(.$input.cost))[1]]
        .$input.cost[is.na(.$input.cost)] <- first_valid
        .
      } ->
      L2247.GlobalTechCost_ptc_USA

...
module_water_L2233.electricity_water <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    ### if na in L223_data$StubTechFixOut_hydro
    if  (any(is.na(as.matrix(L223_data$StubTechFixOut_hydro)))){
      tmp <- L223_data$StubTechFixOut_hydro
      tmp_2015 <- tmp[tmp$year == 2021, ]
      tmp_2015$year <- 2015
      tmp_2015$share.weight.year <- 2015
      tmp <-  tmp[tmp$year != 2015, ]
      L223_data$StubTechFixOut_hydro  <- bind_rows(tmp_2015,tmp)
      }

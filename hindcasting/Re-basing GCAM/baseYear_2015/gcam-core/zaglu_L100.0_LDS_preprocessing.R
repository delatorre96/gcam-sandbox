  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    for(nm in namelist) {
      if("year" %in% names(LDSfiles[[nm]])) {
        ...
        }else if (max_year > MODEL_FINAL_BASE_YEAR) {

          warning(
            "module_aglu_L100.0_LDS_preprocessing: latest year in ", nm," (", max_year, ") is later than the final base-year (",MODEL_FINAL_BASE_YEAR, "). Truncating data."
          )
        ...
        # if max_year == MODEL_FINAL_BASE_YEAR: do nothing
      }
    }
